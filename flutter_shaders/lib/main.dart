...
class _MyHomePageState extends State<MyHomePage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    SchedulerBinding.instance!.addPostFrameCallback((_) {
      var p = FastPoissonDiskSampling(shape: Size((MediaQuery.of(context).size.width - 50).ceil().toDouble(), (MediaQuery.of(context).size.height - 210).ceil().toDouble()), radius: 190, maxTries: 50, minDistance: 0, rng: null);
  ...
  bool checkCollision(RenderBox box1, Size box2Size, Offset box2Offset) {
    ...
    if (containerKey1.currentContext != null) {
      ...
      final collide = (position1.dx < position2.dx + size2.width && position1.dx + size1.width > position2.dx && position1.dy < position2.dy + size2.height && position1.dy + size1.height > position2.dy);
  ...
  bool rectOverlap(Rect A, Rect B) {
    bool xOverlap = valueInRange(A.center.dx.toInt(), B.center.dx.toInt(), B.center.dx.toInt() + (B.width * 0.5).toInt()) || valueInRange(B.center.dx.toInt(), A.center.dx.toInt(), A.center.dx.toInt() + (A.width * 0.5).toInt());

    bool yOverlap = valueInRange(A.center.dy.toInt(), B.center.dy.toInt(), B.center.dy.toInt() + (B.height * 0.5).toInt()) || valueInRange(B.center.dy.toInt(), A.center.dy.toInt(), A.center.dy.toInt() + (A.height * 0.5).toInt());
  ...
  List<Widget> getShapes(BoxConstraints viewportConstraints) {
    ...
    currentNodes.forEach((key, value) {
      Widget widgetObj = Positioned(
          ...
          child: CustomPaint(
            key: value["key"],
            painter: ShapeMaster(type: value["shape"], size: Size(40, 40), radius: 40.0, center: Offset(0, 0), angle: null, color: currentNodes[value["key"].toString()]!["color"] as Color),
            child: Container(),
          ));
  ...
  void generateRandomShapes(List<List<double>> pointsList) {
    ...
    for (var i = 0; i < pointsList.length; i++) {
      ...
      finalList.putIfAbsent(_key.toString(), () => {"key": _key, "isFound": false, "color": Colors.white70, "widgetObj": null, "size": Size(40, 40), "offset": Offset(x, y), "shape": shape, "matched": false});
                          ...
                          Transform.translate(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    boxShadow: [BoxShadow(blurRadius: 8, offset: const Offset(0, 0), color: Colors.black, spreadRadius: 2), BoxShadow(blurRadius: 8, offset: const Offset(0, 0), color: Colors.black, spreadRadius: 2)]),
                                child: Container(
                                    width: 50,
                                    height: 50,
                                    decoration: BoxDecoration(
