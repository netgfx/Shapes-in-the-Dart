...
class Body{
     ...
     * @readonly
     */
    this.halfWidth = abs(sprite.width / 2);

    /**
     * @property {number} halfHeight - The calculated height / 2 of the physics body.
     * @readonly
     */
    this.halfHeight = abs(sprite.height / 2);

    /**
     * @property {Phaser.Point} center - The center coordinate of the Physics Body.
     * @readonly
     */
    this.center = Point<double>(sprite.x + this.halfWidth, sprite.y + this.halfHeight);
     ...
     * @default
     */
    this.maxVelocity = Point(10000, 10000);
     ...
     */
    moveFrom(int duration, int speed, int direction){
     ...
     * @return {boolean} True if the movement successfully started, otherwise false.
     */
    moveTo(int duration, int distance, int? direction){
        ...
        if (direction == null){
            angle = this.angle;
            direction = Utils.shared.radToDeg(angle.toDouble()).toInt();
        }
        else{
            angle = Utils.shared.degreesToRadians(direction.toDouble());
        }
        ...
        if (this.moveTarget == null){
            this.moveTarget = Line();
            this.moveEnd = Point(0,0);
        }

        this.moveTarget.fromAngle(this.x, this.y, angle, distance);

        this.moveEnd = Point<double>(this.moveTarget.end.x, this.moveTarget.end.y);
     ...
     * Sprite's texture.
     */
    setSize(double width, double height, double offsetX, double offsetY){
     ...
     */
    setCircle(int radius, double offsetX, double offsetY){
        if (offsetX === undefined) { offsetX = this.offset.x; }
        if (offsetY === undefined) { offsetY = this.offset.y; }
     ...
     */
    hitTest(x, y){
        return (this.isCircle) ? Utils.shared.contains(this, x, y) : Utils.shared.containsRect(this, x, y);
    }
...
set x(double value) {
  this.position = Point<double>(value, this.position.y);
}
...
set y(double value) {
  this.position = Point<double>(this.position.x, value);
}
 ...
 */
// void renderBodyInfo(debug, body)
// {
